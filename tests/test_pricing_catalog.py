...
class PricingCatalogTests(unittest.TestCase):
    ...
    def test_model_management_ui_contract(self):
        ...
        self.assertIn('id="modelAddDescription"', html_source)
        self.assertIn("['others', uiText('model.add.providerOther'", app_source)
        self.assertIn("data-settings-edit", app_source)
        self.assertIn("pendingModelEditId", app_source)
        self.assertIn("existing.name = name", app_source)
        self.assertIn('"model.edit.title"', (root / "web" / "locales" / "zh-CN.js").read_text(encoding="utf-8"))
